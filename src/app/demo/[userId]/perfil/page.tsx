...
function PinScreen({ userId, mode, onSuccess, onCancel, dark }: {
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col items-center justify-center px-8 bg-[#0a0f1e]">
      ...
      <h2 className="text-xl font-bold mb-1 text-white">{titles[step]}</h2>
      {error
      ...
      <div className="flex gap-5 mb-10">
        {[0,1,2,3].map(i => (
          <div key={i}
            style={{ backgroundColor: i < pin.length ? '#3b82f6' : 'transparent', borderColor: i < pin.length ? '#3b82f6' : '#334155' }}
        ...
        {['1','2','3','4','5','6','7','8','9','','0','⌫'].map((d, i) => (
          ...
          d === '⌫' ? (
            <button key={i} onClick={() => { setPin(p => p.slice(0, -1)); setError('') }}
              className="h-16 rounded-2xl text-xl flex items-center justify-center active:scale-95 transition-all bg-slate-800 text-slate-300">{d}</button>
          ) : (
            <button key={i} onClick={() => handleDigit(d)}
              className="h-16 rounded-2xl text-2xl font-semibold flex items-center justify-center active:scale-95 transition-all bg-slate-800 text-white border border-slate-700">{d}</button>
          )
        ))}
      </div>
...
const PLAN_META = {
  pro:      { label: 'Pro',      gradient: 'from-blue-500 to-indigo-600',    text: 'text-blue-400',   price: '$12.99/mes' },
  business: { label: 'Business', gradient: 'from-violet-500 to-purple-700',  text: 'text-violet-400', price: '$29.99/mes' },
  free:     { label: 'Free',     gradient: 'from-slate-500 to-slate-700',    text: 'text-slate-400',  price: 'Gratis' },
}
...
export default function PerfilPage({ params }: { params: Promise<{ userId: string }> }) {
  ...
  return (
    <div className="min-h-dvh bg-[#0a0f1e] flex flex-col">
      ...
      {/* ── Header fijo ── */}
      <div className="sticky top-0 z-10 flex items-center justify-between px-4 bg-[#0a0f1e]/90 backdrop-blur-sm"
        ...
        <p className="text-sm font-semibold text-white/80">Mi perfil</p>
        <div className="w-9" />
      </div>
      ...
      <div className="flex-1 overflow-y-auto pb-32 px-4 space-y-4">
        ...
        <div className="relative rounded-3xl overflow-hidden bg-gradient-to-br from-blue-600/20 via-indigo-600/10 to-slate-900 border border-white/10 p-6">
          {/* Fondo decorativo */}
          ...
          <div className="relative flex items-center gap-5">
            ...
            <div className="relative shrink-0">
              <div className="w-20 h-20 rounded-2xl overflow-hidden ring-2 ring-blue-500/40 shadow-xl shadow-blue-900/40">
                {me.avatar_url
            ...
            <div className="flex-1 min-w-0">
              ...
              {me.phone && <p className="text-sm text-slate-400 mt-0.5">{me.phone}</p>}
              <p className="text-xs text-slate-500 mt-1">DO Chat · {plan.price}</p>
            </div>
          </div>
          ...
          <div className="relative mt-5 grid grid-cols-3 gap-2">
            {[
              ...
              { label: dark ? 'Modo claro' : 'Modo oscuro', icon: dark
              ...
              btn.href
                ? <Link key={i} href={btn.href}
                    className="flex flex-col items-center gap-1.5 py-3 rounded-2xl bg-white/8 border border-white/10 active:scale-95 transition-all hover:bg-white/12">
                    <span className="text-white/70">{btn.icon}</span>
                    <span className="text-[10px] font-medium text-white/50">{btn.label}</span>
                  </Link>
                : <button key={i} onClick={btn.action} disabled={btn.loading}
                    className="flex flex-col items-center gap-1.5 py-3 rounded-2xl bg-white/8 border border-white/10 active:scale-95 transition-all hover:bg-white/12 disabled:opacity-40">
                    <span className="text-white/70">{btn.loading ? <div className="w-4 h-4 border-2 border-white/30 border-t-white/80 rounded-full animate-spin" /> : btn.icon}</span>
                    <span className="text-[10px] font-medium text-white/50">{btn.label}</span>
                  </button>
            ))}
          </div>
        ...
        <div className="rounded-3xl bg-white/5 border border-white/10 overflow-hidden">
          <div className="flex items-center gap-3 px-5 pt-5 pb-3">
            ...
            <div className={`ml-auto text-[10px] font-semibold px-2.5 py-1 rounded-full ${hasPin ? 'bg-emerald-500/20 text-emerald-400' : 'bg-slate-700 text-slate-400'}`}>
          ...
          <div className="px-4 pb-4 grid grid-cols-2 gap-2">
            {!hasPin ? (
              <button onClick={() => setPinMode('set')}
                className="col-span-2 flex items-center justify-center gap-2 py-3 rounded-2xl bg-blue-600/20 border border-blue-500/30 text-blue-400 text-sm font-semibold active:scale-95 transition-all">
              ...
              <>
                <button onClick={() => setPinMode('change')}
                  className="flex items-center justify-center gap-2 py-3 rounded-2xl bg-amber-500/10 border border-amber-500/20 text-amber-400 text-xs font-semibold active:scale-95 transition-all">
                ...
                <button onClick={() => setPinMode('remove')}
                  className="flex items-center justify-center gap-2 py-3 rounded-2xl bg-red-500/10 border border-red-500/20 text-red-400 text-xs font-semibold active:scale-95 transition-all">
        ...
        <button onClick={() => setShowIcal(v => !v)}
          className="w-full rounded-3xl bg-white/5 border border-white/10 overflow-hidden text-left active:scale-[0.98] transition-all">
          <div className="flex items-center gap-3 px-5 py-4">
            ...
            <div className="flex-1">
              ...
              <p className="text-xs text-slate-500 mt-0.5">{icalUrl ? '✓ Conectado' : 'Conecta tu calendario'}</p>
            </div>
            <svg className={`w-4 h-4 text-slate-500 transition-transform duration-200 ${showIcal ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" d="m19.5 8.25-7.5 7.5-7.5-7.5" />
            </svg>
          </div>
        </button>
        {showIcal && (
          <div className="rounded-3xl bg-white/5 border border-white/10 px-5 py-4 space-y-3">
            <input
              type="url"
              value={icalUrl}
              onChange={e => setIcalUrl(e.target.value)}
              placeholder="https://calendar.google.com/calendar/ical/…"
              className="w-full bg-white/8 border border-white/10 rounded-2xl px-4 py-3 text-sm text-white placeholder:text-slate-600 focus:outline-none focus:border-blue-500 transition-colors"
            />
            <button onClick={saveIcal}
              className={`w-full py-3 rounded-2xl text-sm font-semibold transition-all active:scale-95 ${icalSaved ? 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/30' : 'bg-gradient-to-r from-blue-500 to-indigo-600 text-white shadow-lg shadow-blue-500/20'}`}>
        ...
        <Link href="/pricing"
          ...
          <div className="rounded-[calc(1.5rem-1px)] bg-[#0a0f1e] px-5 py-4 flex items-center gap-4">
            ...
            <div className="flex-1">
              <p className="text-sm font-semibold text-white">Plan {plan.label}</p>
        ...
        <div className="rounded-3xl bg-white/5 border border-white/10 overflow-hidden">
          ...
          <div className="px-4 pb-4 space-y-2">
            <button onClick={handleExport} disabled={exporting}
              className="w-full flex items-center gap-3 px-4 py-3 rounded-2xl bg-blue-600/10 border border-blue-500/20 active:scale-95 transition-all disabled:opacity-40">
              <svg className="w-4 h-4 text-blue-400 shrink-0" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M3 16.5v2.25A2.25 2.25 0 0 0 5.25 21h13.5A2.25 2.25 0 0 0 21 18.75V16.5M16.5 12 12 16.5m0 0L7.5 12m4.5 4.5V3" /></svg>
              <p className="text-sm font-medium text-blue-400">{exporting ? 'Exportando…' : 'Exportar mis datos'}</p>
            </button>
            <button onClick={handleDelete} disabled={deleting}
              className="w-full flex items-center gap-3 px-4 py-3 rounded-2xl bg-red-500/10 border border-red-500/20 active:scale-95 transition-all disabled:opacity-40">
              <svg className="w-4 h-4 text-red-400 shrink-0" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0" /></svg>
              <p className="text-sm font-medium text-red-400">{deleting ? 'Eliminando…' : 'Eliminar cuenta'}</p>
            </button>
          </div>
        </div>
        ...
        <div className="flex gap-4 justify-center py-2">
          <Link href="/privacy" className="text-xs text-slate-600 hover:text-blue-400 transition-colors">Privacidad</Link>
          <span className="text-slate-700">·</span>
          <Link href="/terms" className="text-xs text-slate-600 hover:text-blue-400 transition-colors">Términos</Link>
          <span className="text-slate-700">·</span>
...
function BottomNav({ userId, active }: { userId: string; active: string }) {
  ...
  return (
    <div className="bg-[#0a0f1e]/95 backdrop-blur-md border-t border-white/8 flex fixed bottom-0 left-0 right-0 z-10"
      style={{ paddingBottom: 'env(safe-area-inset-bottom, 0px)' }}>
      {tabs.map(tab => {
        const isActive = active === tab.id
        return (
          <Link key={tab.id} href={tab.href}
            className="flex-1 flex flex-col items-center pt-1.5 pb-1 gap-0.5 transition-colors">
            <div className={`flex items-center justify-center w-12 h-7 rounded-full transition-all duration-300 ${isActive ? 'bg-blue-500/20' : ''}`}>
              <span className={`transition-colors duration-200 ${isActive ? 'text-blue-400' : 'text-slate-600'}`}>
                {tab.icon}
              </span>
            </div>
