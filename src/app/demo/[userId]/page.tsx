...
const [darkMode, setDarkMode] = useState(() => {
  ...
  useEffect(() => {
    ...
    fetch('/api/auth/profile').then(r => r.json()).then(d => {
      if (d.profile) {
        ...
        if (upgraded) {
          ...
          window.history.replaceState({}, '', `/demo/${userId}`)
    ...
    fetch('/api/demo/agents', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ user_id: userId }) })
...
setDarkMode(localStorage.getItem('dark_mode') === '1')
  ...
  async function fetchRooms() {
    try {
      const res = await fetch(`/api/demo/chat-list?user_id=${userId}`, { cache: 'no-store' })
  ...
  async function ncSave() {
    ...
    try {
      ...
      const res = await fetch('/api/demo/contacts', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) })
      const data = await res.json()
  ...
  async function addTask() {
    ...
    setAddingTask(true)
    try {
      const res = await fetch('/api/demo/tasks', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ user_id: userId, content: optimisticTask.content, due_date: optimisticTask.due_date }) })
  ...
  async function createProject() {
    const res = await fetch('/api/demo/projects', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ user_id: userId, title: 'New project', instructions: '', project_files: [] }) })
      ...
      {upgradeToast && (
        <div className="fixed top-4 left-1/2 -translate-x-1/2 z-50 w-[calc(100%-2rem)] max-w-sm">
          <div className="bg-emerald-600 text-white rounded-2xl shadow-xl px-4 py-3 flex items-center gap-3">
            ...
            <button onClick={() => setUpgradeToast(null)} className="text-white/70 hover:text-white ml-auto shrink-0">
                ...
                <div className="px-4 pt-4 pb-2">
                  <button onClick={() => setActiveRoomId(aiRoom.id)}
                    ...
                    <div className="flex-1 min-w-0 text-left">
                      ...
                      <p className="text-white/70 text-xs truncate">{aiRoom.lastMsg?.content ?? a.chats.aiSubtitle}</p>
                    </div>
        ...
        <div className="relative shrink-0">
          {room.otherAvatarUrl
            ...
            : <div className={`w-12 h-12 rounded-2xl flex items-center justify-center text-xl shadow-sm ${room.type === 'group' ? 'bg-gradient-to-br from-violet-500 to-purple-700' : room.type === 'dm' ? 'bg-gradient-to-br from-slate-600 to-slate-800' : 'bg-gradient-to-br from-blue-500 to-blue-700'}`}>
                {room.emoji ? <span className="text-xl">{room.emoji}</span> : <UserIcon className="w-6 h-6 text-white/80" />}
              </div>
          }
          {hasUnread && (
