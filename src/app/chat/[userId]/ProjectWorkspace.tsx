...
export function ProjectWorkspace({ userId, project: initial, onClose, onUpdate }: {
  ...
  async function fetchMessages() {
    const res = await fetch(`/api/chat/messages?room=${roomId}&user_id=${userId}`)
  ...
  async function uploadFile(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (data.message?.content) {
      try {
        ...
        await fetch('/api/chat/projects', { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id: project.id, project_files: updated.project_files }) })
  ...
  async function saveMeta() {
    ...
    await fetch('/api/chat/projects', { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id: project.id, title: updated.title, instructions: updated.instructions }) })
    setProject(updated); onUpdate(updated)
