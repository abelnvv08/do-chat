                ...
                <div key={roomId}>
                  ...
                  {msgs.map(result => (
                    <button key={result.id} onClick={() => { setSearchQuery(''); setActiveRoomId(result.room_id) }}
                      ...
                      <div className={`w-9 h-9 shrink-0 flex items-center justify-center text-sm mt-0.5 ${result.type === 'ai' ? 'rounded-xl bg-gradient-to-br from-blue-400 to-indigo-600 text-white shadow-sm' : 'rounded-full bg-slate-100'}`}>{result.type === 'ai' ? '✦' : result.sender_emoji}</div>
                      <div className="flex-1 min-w-0">
      ...
      {activeTab === 'archivos' && (() => {
        ...
        async function deleteSelected() {
          ...
          for (const id of toDelete) {
            ...
            if (proj) { await fetch('/api/chat/projects', { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id }) }); setProjects(prev => prev.filter(p => p.id !== id)) }
            const file = files.find(f => f.id === id)
            if (file) { await fetch('/api/chat/messages', { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ message_id: id }) }); setFiles(prev => prev.filter(f => f.id !== id)) }
          }
