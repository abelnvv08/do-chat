...
export function RoomView({ userId, roomId, onBack, initialRoom, autoCall, onCallLog }: { userId: string; roomId: string; onBack: () => void; initialRoom?: { id: string; name: string; emoji: string; type: string; otherUserId?: string; otherAvatarUrl?: string | null } | null; autoCall?: boolean; onCallLog?: (log: CallLogEntry) => void }) {
  ...
  async function sendTask() {
    ...
    const myName = me?.name ?? messages.find(m => m.user_id === userId)?.user?.name ?? 'Yo'
    const myEmoji = me?.emoji ?? messages.find(m => m.user_id === userId)?.user?.emoji ?? '👤'
    const taskRes = await fetch('/api/chat/tasks', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        user_id: userId,
      ...
      <div className="px-4 pb-3 sticky top-0 z-10" style={{ paddingTop: 'calc(env(safe-area-inset-top, 44px) + 12px)', background: 'linear-gradient(135deg, #1e40af 0%, #2563eb 100%)' }}>
        {showSearch ? (
          ...
          <div className="flex items-center gap-3">
            <button onClick={onBack} aria-label="Volver" className="flex items-center gap-2 text-white/80 hover:text-white transition-colors p-1 -ml-1">
            ...
            <div className="w-10 h-10 rounded-2xl shrink-0 overflow-hidden flex items-center justify-center">
              {isAIRoom
                ...
                : roomData.otherAvatarUrl
                  ...
                  : roomData.type === 'group'
                    ? <div className="w-10 h-10 bg-[#707070] flex items-center justify-center"><svg className="w-5 h-5 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4" strokeWidth={1.5}/><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M23 21v-2a4 4 0 0 0-3-3.87"/><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M16 3.13a4 4 0 0 1 0 7.75"/></svg></div>
                    : <div className="w-10 h-10 bg-[#707070] flex items-center justify-center"><svg className="w-5 h-5 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/></svg></div>
              }
            </div>
